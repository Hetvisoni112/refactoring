...
public class StatementPrinter {
    ...
    public String statement() {
        ...
        for (Performance performance : invoice.getPerformances()) {
            ...
            final int getAmount = getThisAmount(performance);
            ...
            result.append(String.format("  %s: %s (%s seats)%n", getPlay(performance).getName(),
                    format.format(getAmount / Constants.PERCENT_FACTOR), performance.getAudience()));
        ...
        switch (getPlay(performance).getType()) {
            case "tragedy":
                ...
                if (performance.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                    final int secondTerm = performance.getAudience() - Constants.BASE_VOLUME_CREDIT_THRESHOLD;
                    thisAmount += Constants.HISTORY_OVER_BASE_CAPACITY_PER_PERSON * secondTerm;
                }
                break;
            case "comedy":
